...
import { RSTestSetup } from "./RSTest.t.sol";
import {
  LibRaidRoles as Lib,
  Roles,
  RaidData,
...
contract MaskTest is RSTestSetup {
...
contract AddToTest is RSTestSetup {
  function test_addTo() public {
    Roles role;
    uint16 roles_;
    for (uint256 i; i < NUM_ROLES; ++i) {
      role = Roles(i);
      roles_ = Lib.addTo(role, roles_);
      assertTrue(Lib.isIn(role, roles_));
    }
  }
...
contract KeyTest is RSTestSetup {
